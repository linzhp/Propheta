...
public class OutputResultPDF {
	...
	public void saveResult(){
		...
		try {
			...
			Runtime.getRuntime().exec("cmd /C "+FormatUtility.splitString(filepath));
		} catch (IOException e) {
    ...
    public void saveBasicInfo(){
		...
		// 开发平台
		resultFormat("开发平台", fu.ConvertKeyToShow(4, nb.get("developmentPlatform")));
		// 开发技术
		resultFormat("开发技术", fu.ConvertKeyToShow(3, nb.get("developmentTechniques")));
		// 语言类型
    ...
    public void saveQuickInfo(){
			...
			pdf.addParagra(0, "根据公式计算出来的工作量为："
					+ nb.get("formulaEffort") + "小时");			
			//historyEffort_out = qer.getHistoryEffort();
			pdf.addParagra(0, "根据历史项目数据的生产率中位数值计算出的工作量：："
					+ nb.get("historyEffort") + "小时");
			//输出蒙特卡罗图			
			...
			else
			{
				...
				JFreeChart monteCarloChart = Chart.createXYLineChart("工作量蒙特卡罗图", "工时", null, Chart
						.createQuickDataSet(nb.get("dataType").toString(), projectSize,
								(Double)nb.get("meanProductivity"), (Double)nb.get("stanDevProductivity")));
    ...
    public void saveCocomoResult(){
		...
		JFreeChart phaseEffortBarChart = Chart.createBarChart("阶段工作量分布", "阶段",
				"工作量(人.月)", Chart.createCategoryDataset(phasesTex, phaseEfforts));
